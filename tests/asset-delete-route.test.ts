...
const mocks=vi.hoisted(()=>({auth:vi.fn(),find:vi.fn(),references:vi.fn(),deleteRecord:vi.fn(),deleteRemote:vi.fn()}));
vi.mock("@/auth",()=>({auth:mocks.auth}));
vi.mock("@/features/assets/server/asset.repository",()=>({findAssetByIdAndOwner:mocks.find,findAssetProjectReferences:mocks.references,deleteAssetRecord:mocks.deleteRecord}));
