...
const mocks=vi.hoisted(()=>({auth:vi.fn(),preflight:vi.fn(),publish:vi.fn(),unpublish:vi.fn()}));
...
const request=(body="{}",headers:Record<string,string>={})=>new Request("http://test/api/projects/project-a/publish",{method:"POST",headers:{"content-type":"application/json",...headers},body});

describe("publication route ownership and preflight",()=>{
  beforeEach(()=>{mocks.auth.mockResolvedValue({user:{id:"owner-a"}});mocks.preflight.mockResolvedValue({success:true,project:{id:"project-a"},website:initialWebsite,slug:"my-site"});mocks.publish.mockResolvedValue({slug:"my-site",isPublished:true,publishedAt:new Date("2026-01-01")});mocks.unpublish.mockResolvedValue({count:1})});
  ...
  it("blocks unresolved local assets",async()=>{mocks.preflight.mockResolvedValue({success:false,status:422,body:{error:"Upload local legacy assets before publishing.",unresolvedAssetCount:1}});const response=await POST(request(),context);expect(response.status).toBe(422);expect((await response.json()).unresolvedAssetCount).toBe(1)});
  it("publishes only the validated preflight snapshot",async()=>{const response=await POST(request(),context);expect(response.status).toBe(200);expect(mocks.publish).toHaveBeenCalledWith("project-a","owner-a","my-site",initialWebsite)});
