...
vi.mock("@/features/projects/server/project.repository",()=>({
  LivePublicationLimitError: class LivePublicationLimitError extends Error { constructor(public liveProject:{id:string;name:string;slug:string|null}){super("Only one website can be live at a time.")} },
  SlugConflictError: class SlugConflictError extends Error {},
  publishProject:mocks.publish,
  unpublishProject:mocks.unpublish,
}));
...
import { LivePublicationLimitError } from "@/features/projects/server/project.repository";

const context={params:Promise.resolve({projectId:"project-a"})};
const request=(body=JSON.stringify({title:"My site",iconUrl:null,iconData:null}),headers:Record<string,string>={})=>new Request("http://test/api/projects/project-a/publish",{method:"POST",headers:{"content-type":"application/json",...headers},body});
...
describe("publication route ownership and preflight",()=>{
  ...
  it("publishes only the validated preflight snapshot",async()=>{const response=await POST(request(),context);expect(response.status).toBe(200);expect(mocks.publish).toHaveBeenCalledWith("project-a","owner-a","my-site",initialWebsite,4,{title:"My site",iconUrl:null,iconData:null})});
  it("requires the owner's existing live website to be unpublished first",async()=>{mocks.publish.mockRejectedValue(new LivePublicationLimitError({id:"project-live",name:"Live portfolio",slug:"live-portfolio"}));const response=await POST(request(),context);expect(response.status).toBe(409);expect(await response.json()).toMatchObject({error:{code:"LIVE_PUBLICATION_LIMIT",details:{liveProject:{id:"project-live"}}}})});
  it("rejects an icon that is not an active asset owned by the project",async()=>{mocks.findIcon.mockResolvedValue(null);const response=await POST(request(JSON.stringify({title:"My site",iconUrl:"https://assets.test/icon.png",iconData:null})),context);expect(response.status).toBe(400);expect(mocks.publish).not.toHaveBeenCalled()});
  it("returns 400 for malformed JSON and 413 for oversized bodies",async()=>{expect((await POST(request("{"),context)).status).toBe(400);expect((await POST(request("{}",{"content-length":"900000"}),context)).status).toBe(413)});
