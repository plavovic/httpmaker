...
describe("publication route ownership and preflight",()=>{
  ...
  it("publishes only the validated preflight snapshot",async()=>{const response=await POST(request(),context);expect(response.status).toBe(200);expect(mocks.publish).toHaveBeenCalledWith("project-a","owner-a","my-site",initialWebsite,4)});
