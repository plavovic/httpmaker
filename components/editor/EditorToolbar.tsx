...
export default function EditorToolbar(props: Props) {
  return <header className="studio-toolbar">
    <div className="studio-project"><a href="/dashboard" className="studio-project-mark studio-profile-mark" aria-label="Open dashboard" title="Open dashboard">{props.profileImage ? <img src={props.profileImage} alt="" /> : <span>{props.profileName.slice(0, 1).toUpperCase()}</span>}</a><div><div className="studio-project-path"><a href="/dashboard">Projects</a><b>/</b> {props.projectName}</div><div className="studio-save-state"><i /> {props.saveState}</div></div><span className="studio-project-divider" aria-hidden="true" /><label className="studio-theme-picker"><span className="sr-only">Studio theme</span><select value={props.colorMode} onChange={(event) => props.onColorModeChange(event.target.value as ColorMode)}>{STUDIO_THEMES.map((theme) => <option key={theme.value} value={theme.value}>{theme.label}</option>)}</select></label></div>
