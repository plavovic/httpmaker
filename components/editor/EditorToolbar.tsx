...
type Props = { colorMode: ColorMode; onToggleColorMode: () => void; viewMode: ViewMode; onViewModeChange: (mode: ViewMode) => void; onOpenPreview: () => void; editorTab: EditorTab; onEditorTabChange: (tab: EditorTab) => void; device: DeviceMode; onDeviceChange: (device: DeviceMode) => void; canUndo:boolean; canRedo:boolean; onUndo:()=>void; onRedo:()=>void };
...
export default function EditorToolbar(props: Props) {
  return <header className="studio-toolbar">
    ...
    <div className="studio-actions">
      <div className="studio-history-controls" aria-label="Edit history"><button type="button" onClick={props.onUndo} disabled={!props.canUndo} aria-label="Undo" title="Undo (Ctrl+Z)">↶</button><button type="button" onClick={props.onRedo} disabled={!props.canRedo} aria-label="Redo" title="Redo (Ctrl+Shift+Z)">↷</button></div>
