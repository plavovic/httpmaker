...
import AiProposalPanel from "@/components/editor/ai/AiProposalPanel";
...
type EditorSidebarProps = { messages: ChatMessage[]; history: PromptHistoryItem[]; isProcessing: boolean; prompt: string; onPromptChange: (value: string) => void; autoMode: boolean; onToggleAutoMode: () => void; onSubmit: (event: FormEvent) => void; proposal: AiPatchProposal | null; onApplyProposal: () => void; onDiscardProposal: () => void };
...
export default function EditorSidebar(props: EditorSidebarProps) {
  ...
  const resize = (event: PointerEvent<HTMLDivElement>) => { if (event.currentTarget.hasPointerCapture(event.pointerId)) setWidth(Math.max(320, Math.min(event.clientX, window.innerWidth / 3))); };
  ...
  return <aside className="ide-sidebar relative flex w-full flex-col border-r border-zinc-200 bg-[#fcfbf7]" style={{ width, maxWidth: "33.333vw" }}>
    <div className="ai-chat-header border-b border-zinc-200 p-5"><div className="relative flex items-center justify-end"><h2 className="brand-title text-xl font-semibold text-zinc-900">HTTPMAKER</h2><button type="button" onClick={() => setHistoryOpen((open) => !open)} className="history-toggle rounded-full border px-3 py-1 text-xs font-medium" aria-expanded={historyOpen}>History</button></div></div>
    ...
    {props.proposal && <div className="ai-proposal-slot"><AiProposalPanel proposal={props.proposal} onApply={props.onApplyProposal} onDiscard={props.onDiscardProposal} /></div>}
