...
function mapWebsiteImages(website: WebsiteJSON, mapSource: (source: string) => string): WebsiteJSON {
 return {...website,sections:website.sections.map(section=>({...section,props:{...section.props,imageUrl:mapSource(section.props.imageUrl),items:section.props.items?.map(mapSource)}}))};
}
...
export const listImageAssets=()=>transaction<UploadedImageAsset[]>("readonly",(store,resolve,reject)=>{const request=store.getAll();request.onsuccess=()=>resolve((request.result as UploadedImageAsset[]).sort((a,b)=>b.createdAt-a.createdAt));request.onerror=()=>reject(request.error)});
export const saveImageAsset=(asset:UploadedImageAsset)=>transaction<void>("readwrite",(store,resolve,reject)=>{const request=store.put(asset);request.onsuccess=()=>resolve();request.onerror=()=>reject(request.error)});
export const deleteImageAsset=(id:string)=>transaction<void>("readwrite",(store,resolve,reject)=>{const request=store.delete(id);request.onsuccess=()=>resolve();request.onerror=()=>reject(request.error)});
...
export async function createImageAsset(file:File):Promise<UploadedImageAsset>{
 ...
 return{id:crypto.randomUUID(),name:file.name,mimeType:file.type,size:file.size,...dimensions,dataUrl,createdAt:Date.now()};
}
