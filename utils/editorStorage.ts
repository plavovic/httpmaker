...
export function readStoredWebsite(): WebsiteJSON | null { try { const value: unknown = JSON.parse(localStorage.getItem(WEBSITE_STORAGE_KEY) ?? "null"); if (!value || typeof value !== "object" || !("theme" in value) || !("sections" in value) || !Array.isArray(value.sections)) return null; return normalizeWebsite(value as WebsiteJSON); } catch { return null; } }
