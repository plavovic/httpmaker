...
type Details = { project: { id:string;name:string;slug:string|null;isPublished:boolean;updatedAt:string;publishedAt:string|null;draftRevision:number;publishedRevision:number|null;publicationTitle:string|null;publicationIconUrl:string|null;publicationIconData:string|null }; preflight:{draftValid:boolean;unresolvedAssetCount:number}; publicationLimit:{otherLiveProject:{id:string;name:string;slug:string|null}|null} };
...
export default function PublishClient({projectId}:{projectId:string}) {
  ...
  const [title,setTitle]=useState(""); const [originalTitle,setOriginalTitle]=useState(""); const [iconUrl,setIconUrl]=useState<string|null>(null); const [iconData,setIconData]=useState<string|null>(null); const [originalIconData,setOriginalIconData]=useState<string|null>(null); const [iconUploading,setIconUploading]=useState(false);
  ...
  const load=useCallback(async()=>{try{const body=await readApiResponse<Details>(await fetch(`/api/projects/${encodeURIComponent(projectId)}/publish`));if(body){setDetails(body);if(!initialized.current){const next=body.project.slug??"";const nextTitle=body.project.publicationTitle??body.project.name;setSlug(next);setOriginalSlug(next);setTitle(nextTitle);setOriginalTitle(nextTitle);setIconUrl(body.project.publicationIconUrl);setIconData(body.project.publicationIconData);setOriginalIconData(body.project.publicationIconData);initialized.current=true}}}catch(reason){setMessage(reason instanceof Error?reason.message:"Unable to load publishing details.")}},[projectId]);
  useEffect(()=>{void load()},[load]);
  const normalized=slug.trim(); const validation=slugSchema.safeParse(normalized); const normalizedTitle=title.trim(); const changed=normalized!==originalSlug||normalizedTitle!==originalTitle||iconData!==originalIconData;
  ...
  const otherLiveProject=details.publicationLimit.otherLiveProject;
  const canSubmit=!otherLiveProject&&validation.success&&normalizedTitle.length>0&&normalizedTitle.length<=120&&availability==="available"&&!busy&&!iconUploading&&(hasChanges||changed||!project.isPublished);
  const mutate=async()=>{if(!canSubmit)return;if(project.isPublished&&normalized!==originalSlug&&!window.confirm(`Change the public URL?\n\nOld: ${window.location.origin}/${originalSlug}\nNew: ${publicUrl}\n\nExisting links to the old URL will stop working.`))return;setBusy(true);setMessage("");try{await readApiResponse(await fetch(`/api/projects/${encodeURIComponent(projectId)}/publish`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({slug:normalized,title:normalizedTitle,iconUrl:iconData?null:iconUrl,iconData,expectedRevision:project.draftRevision})}));setMessage("Publishing details saved.");await load();setOriginalSlug(normalized);setOriginalTitle(normalizedTitle);setOriginalIconData(iconData)}catch(reason){setMessage(reason instanceof ApiResponseError?reason.message:"Unable to publish. Please try again.")}finally{setBusy(false)}};
  const uploadIcon=async(file:File)=>{setIconUploading(true);setMessage("");try{const form=new FormData();form.set("file",file);const body=await readApiResponse<{iconData:string}>(await fetch(`/api/projects/${encodeURIComponent(projectId)}/publish/icon`,{method:"POST",body:form}));if(!body)throw new Error("Icon upload returned no data.");setIconData(body.iconData);setIconUrl(null);setMessage("Icon ready. Publish or update details to make it live.")}catch(reason){setMessage(reason instanceof Error?reason.message:"Unable to upload the icon.")}finally{setIconUploading(false)}};
  ...
  return <main data-theme={isLightStudioTheme(colorMode)?"light":"dark"} data-color-theme={colorMode} className={`${styles.shell} ${themeStyles.themed}`}>
    ...
    <section className={styles.card}>
      ...
      <div className={styles.branding}>
        ...
        <div className={styles.iconField}><span className={styles.iconPreview}>{iconData||iconUrl?<img src={iconData??iconUrl!} alt="Current website icon"/>:<b>{normalizedTitle.slice(0,1).toUpperCase()||"W"}</b>}</span><label>Website icon<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadIcon(file);event.target.value=""}} disabled={iconUploading}/></label><small>Square PNG, JPG, WebP, or GIF. Up to 512 KB.</small>{(iconData||iconUrl)&&<button type="button" onClick={()=>{setIconData(null);setIconUrl(null)}}>Remove icon</button>}</div>
      </div>
      ...
      {otherLiveProject&&<aside className={styles.limitNotice} role="alert"><strong>One live website at a time</strong><p><b>{otherLiveProject.name}</b> is currently live. Unpublish it before publishing this website.</p><Link href={`/dashboard/projects/${otherLiveProject.id}/publish`}>Manage live website →</Link></aside>}
